...
function PartnerRegistration() {
  ...
  const handleSubmitPartner = (e) => {
    e.preventDefault();
    handlePartnerReg(e, partnerFormData);
  };
    ...
    <div>
      <form onSubmit={handleSubmitPartner} className='form-container'>
        ...
        <div className='buttons'>
          ...
          {step < 10 ? <button onClick={handleNext} >Next</button> : <button type="submit" disabled={!partnerFormData.complianceConfirmation || !partnerFormData.termsAgreement}>Finish</button>}
        </div>
      </form>
    </div>
  )
